class Tweet {
  ...
  toFeedItem() {
    ...
    if (this.retweeted_status) {
      // full text on RT
      const rt = this.retweeted_status;
      full_text = 'RT @' + rt.user.screen_name + ': ' + rt.full_text;
    }

    var rich_text = full_text;
    if (this.quoted_status) {
      const q = this.quoted_status;
      rich_text += '<br/>Quote @' + q.user.screen_name + ': ' + q.full_text;
    }
    // clickable urls
    var urlPattern = /(\b(https?):\/\/[-A-Z0-9+&@#\/%?=~_|!:,.;]*[-A-Z0-9+&@#\/%=~_|])/gim;
    rich_text = rich_text.replace(urlPattern, '<a href="$1" target="_blank">$1</a>');

    // media
    if (this.extended_entities && this.extended_entities.media) {
      this.extended_entities.media.forEach(function (media) {
        rich_text += '<br/><a href="' + media.url + '"><img src="' + media.media_url_https + '"/></a>';
      });
    }
    return {
      title: this.user.screen_name + ': ' + full_text,
      description: this.user.screen_name + ': ' + rich_text,
      url: 'https://twitter.com/' + this.user.screen_name + '/status/' + this.id_str,
